...
appsAvailDf = pd.read_excel(inputPath,
                          sheet_name=1)
#appsAvailDf = appsAvailDf.drop(appsAvailDf.columns[1],axis = 1)
#appsAvailDf.columns.values[1] = "Monday, January 16"
#appsAvailDf.columns.values[2] = "before8"
#appsAvailDf.columns.values[3] = "after5"
#cols2Move = list(appsAvailDf.columns.values)
#move1 = cols2Move[2]
#move2 = cols2Move[3]
#cols2Move.pop(2)
#cols2Move.pop(3)
#appsAvailDf = appsAvailDf[cols2Move+[move1,move2]]
...
#facAlumAvailDf.columns.values[1] = "num_interviews"
#facAlumAvailDf.columns.values[2] = "b2bInt1Day"
#facAlumAvailDf.columns.values[3] = "indicate"
#facAlumAvailDf.columns.values[4] = "availMore5"
...
#print(summaryDf['Faculty'])
#print(len(summaryDf['Faculty']))
#25 faculty currently
...
#finalData = {'faculty': np.tile(allFacAlum, 90),
#             'time': np.tile(allTimes, 205),
#             'day': np.tile(allDays, 738),
#             'applicant': np.repeat('open',3690)}
...
for i in appsPoints.keys():
    ...
    #Find first faculty:
    fac1 = appsFacAlumDf.loc[(appsFacAlumDf['Student'] == i),["1"]].iloc[0,0]
    ...
    if((pd.isna(fac1)) | (facsAssignNums.get(fac1) is None)):
        fac1 = appsFacAlumDf.loc[(appsFacAlumDf['Student'] == i),["2"]].iloc[0,0]
    ...
    facTimes = facAlumAvailDf.drop(facAlumAvailDf.columns[[1,2,3,4,10,11,12,13,14,15]],axis=1).loc[(facAlumAvailDf['Your name'] == fac1)]
...
############## major if case here ###########################################                            
                            ...
                            checkPresent = (finalDf[(finalDf['faculty']==fac1) & \
                                            (finalDf['time']==olap[check]) & \
                                            (finalDf['day']==column)]['applicant']\
                                            .to_string(index=False) == 'open') & \
                                            (appsPoints[i] < 2)
                            #get row index of current time slot
                            currentIndex = finalDf.index[(finalDf['faculty']==fac1) & \
                                                        (finalDf['time']==olap[check]) & \
                                                        (finalDf['day']==column)].values[0]
...
############## major if case here ###########################################                            
                                ...
                                #check present time slot:
                                checkPresent = (finalDf[(finalDf['faculty']==fac2) & \
                                            (finalDf['time']==olap[check]) & \
                                            (finalDf['day']==column)]['applicant']\
                                            .to_string(index=False) == 'open') & \
                                            (appsPoints[i] < 2)
                                #get row index of current time slot
                                currentIndex = finalDf.index[(finalDf['faculty']==fac2) & \
                                                        (finalDf['time']==olap[check]) & \
                                                        (finalDf['day']==column)][0]
                                #check 1/2 hour after 8 and 1/2 hour before 5:
                                checkBefore = (not ((finalDf.iloc[currentIndex]['applicant'] == '8:00-8:30') & \
                                    (finalDf.iloc[currentIndex+1]['applicant'] != 'open')))
                                checkAfter = (not ((finalDf.iloc[currentIndex]['applicant'] == '4:30-5:00') & \
...
dfp.to_excel('appsPoints.xlsx',index=False)
...
dfa.to_excel('facNums.xlsx',index=False)
...
#Write to excel:
writer = pd.ExcelWriter("mm_output.xlsx",engine='xlsxwriter') 
startRow = 2
headerRow = 0
...
class qbs_app_kivy(App):
    
    def build(self):
        #f = BoxLayout(orientation='vertical')
        #txt = TextInput(multiline=True, 
        #                cursor_blink=True, 
    ...
    #    #return game
    #    return sm
    #    #return WindowManager(transition=WipeTransition())
